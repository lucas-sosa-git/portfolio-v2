...
void main() {
  ...
  float edgeProgress = smoothstep(
    0.22 + cornerDelay * 0.35,
    0.68 + cornerDelay * 0.35,
    uCollapse
  );
  float cornerStart = mix(0.42, 0.5, uCornerResistance) + cornerDelay;
  float cornerProgress = smoothstep(
    cornerStart,
    cornerStart + mix(0.25, 0.34, uCornerResistance),
    uCollapse
  );
  ...
  axisProgress = clamp(
    ...
    vec3(min(1.0, globalProgress + 0.3))
  );
  ...
  float asymmetryFactor =
    ...
    0.22 *
    remainingStructure;
  float surfaceNoise = noise3(position * 5.8 + vec3(uTime * 0.18));
  float noiseFactor =
    1.0 +
    (surfaceNoise - 0.5) *
    2.0 *
    min(uNoiseMax, 0.1) *
    remainingStructure;
  axisProgress = clamp(axisProgress * asymmetryFactor * noiseFactor, 0.0, 1.0);

  vec3 inward = -position / max(length(position), 0.0001);
  vec3 transformed = position * mix(vec3(1.0), vec3(uFinalScale), axisProgress);
